...
{
    WiFiProvisioner::WiFiProvisioner() : m_server(nullptr), m_dns_server(nullptr), apIP(192, 168, 4, 1), netMsk(255, 255, 255, 0) 
    {
    ...
    void WiFiProvisioner::setShowInputField(bool value) 
    { 
    ...
    void WiFiProvisioner::stopServerLoop(bool value) 
    { 
        stopLoopFlag = value; 
    }
    ...
    void WiFiProvisioner::enableSerialDebug(bool enable) 
    { 
        serialDebug = enable; 
    }
    ...
    void WiFiProvisioner::connectToWiFi(bool is_setup_AP) 
    {
    ...
    bool WiFiProvisioner::connectToExistingWiFINetwork() 
    {
        ...
        if (storedSSID != "") 
        {
            ...
            delay(wifiDelay);
            debugPrintln("Found existing wifi credientials, trying to connect with timeout" + String(connectionTimeout));
            ...
            while (WiFi.status() != WL_CONNECTED) 
            {
                ...
                if (connectionTimeout != 0 && (millis() - startTime) >= connectionTimeout) 
                {
                    WiFi.disconnect();
                    delay(wifiDelay);
                    debugPrintln("Connection timeout reached, continuing to start the provision");
                    return false;
                }
            }
            return true;
    ...
    void WiFiProvisioner::setupAccessPointAndServer() 
    {
        ...
        m_server->on("/configure", HTTP_POST, [this]() { this->handleConfigureRequest(); });
        ...
        m_server->on("/factoryreset", HTTP_POST, [this]() { this->resetToFactorySettings(); });
    ...
    void WiFiProvisioner::connectToWiFiNetwork(const String &ssid, const String &password) 
    {
        if (password.isEmpty()) 
        {
        ...
        while (WiFi.status() != WL_CONNECTED && (millis() - startTime) <= newWifiConnectionTimeout) 
        {
            delay(wifiDelay);
    ...
    void WiFiProvisioner::handleSuccessfulConnection(bool input_ok, const String &ssid, const String &password) 
    {
        if (input_ok) 
        {
            // Save Credientials
    ...
    void WiFiProvisioner::saveNetworkConnectionDetails(const String &ssid, const String &password) 
    {
        m_preferences.begin("network", false);
